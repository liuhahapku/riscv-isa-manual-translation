...
\hl{
  本章描述了机器模式(M-mode)中可用的机器级操作，机器模式是RISC-V系统中的最高
  特权模式。机器模式用于对硬件平台的低级访问，是在复位时进入的第一模式。机器模式
  还可以用于实现那些在硬件中直接实现过于困难或昂贵的功能。RISC-V机器级别的ISA
  包含一个公共的核心部分，可以根据支持的其他权限级别和硬件实现的其他细节进行扩展。
}

\section{Machine-Level CSRs ~\\ \hl{机器级CSRs}}
...
\subsection{Machine ISA Register {\tt misa} ~\\ \hl{机器ISA寄存器{\tt misa}}}
\label{sec:misa}
...
\hl{
  {\tt misa} CSR是一个读任意写合法（WARL）的读写寄存器，记录着硬件线程所支持的
  指令集架构。在所有的实现中，该寄存器必须是可读的。返回值是0意味着{\tt misa}没有
  实现，此时需要通过一个独立的非标准机制来确定CPU的能力。
}
...
\hl{
  {\tt misa}寄存器的MXL(Machine XLEN)字段编码了原生基础整数指令集的宽度，如表~\ref{misabase}
  所示。在支持多种基础指令集的实现中，MXL字段有可能是可写的。机器模式下有效的XLEN,{\em MXLEN},
  是通过设置MXL给出的，或者在{\tt misa}为0时有固定值。当机器重置时，MXL字段总是被设置成
  被最广泛地支持的ISA。
}
...
\hl{
  {\tt misa} CSR的宽度是 MXLEN 比特。如果读出的{\tt misa}是非零值，MXL字段总是
  代表了当前的MXLEN。如果写入{\tt misa}导致MXLEN发生改变，MXL字段的位置就会移动到
  拥有新宽度的{\tt misa}的最高的两位。
}
...
\begin{commentary}
  \hl{
    基础整数寄存器的宽度可以被快速确定，方法是通过判断{\tt misa}值的符号，或者需要先把
    改值左移一位再判断符号。这种检查方法可以写成汇编代码，即使不知道机器的寄存器的宽度（XLEN）。
    基础宽度的计算公式是}$\mbox{XLEN}=2^{\mbox{MXL+4}}$
  
    \indent \hl{如果{\tt misa}的值是 0 ，基础宽度还可以通过另一种方法来确定。通过把立即数4放在
    一个寄存器中，并把该寄存器一次左移31位。如果左移一次后值是0 ，那么机器就是RV32的。如果
    左移两次后值才变成0，那么机器是RV64的。如果还不为0 ，那么机器是RV128的。
  }
\end{commentary}
...
\hl{
  Extensions 字段编码了支持哪些标准扩展，每一位代表字母表中的一个字母（比如第0比特位代表
  “A”扩展，第1比特位代表“B”扩展，一直到第25比特位代表“Z”扩展）。“I”比特位用于表示
  RV32I, RV64I, RV128I 基础指令集，“E”比特位用于表示RV32E。Extensions 字段是
  WARL字段，在支持切换ISA的实现中这些比特是可写的。重置的时候，Extensions 字段
  将包含所支持的最大指令集合，当I和E都可用时，重置会选择I。
}

When a standard extension is disabled by clearing its bit in {\tt misa}, the instructions 
and CSRs defined or modified by the extension revert to their defined or 
reserved behaviors as if the extension is not implemented.

\hl{
  当一个标准扩展在{\tt misa}寄存器中的相应的比特位被清除时，由该扩展定义或者修改的指令和
  CSRs都会回到原来的或者默认的状态，就像这种扩展没有被实现一样。
}
...
\hl{
  RV128I基础指令集现在还没有冻结，虽然本规范大部分剩余部分预计将应用于RV128，但本文档在这个版本只关注于RV32和RV64。
}

The ``U'' and ``S'' bits will be set if there is support for user and
supervisor modes respectively.

\hl{"U"和"S"比特位用于支持用户模式和监管者模式}

The ``X'' bit will be set if there are any non-standard extensions.

\hl{当任意非标准扩展存在时，"X"比特位被置位。}
...
\hline
  0 & A & Atomic extension \hl{原子指令扩展}\\
  1 & B & {\em Tentatively reserved for Bit-Manipulation extension} \\
    &   & \hl{暂定为比特操作扩展保留} \\
  2 & C & Compressed extension \hl{压缩指令扩展}\\
  3 & D & Double-precision floating-point extension \\
  4 & E & RV32E base ISA \\
  5 & F & Single-precision floating-point extension \\
  6 & G & {\em Reserved} \\
  7 & H & Hypervisor extension \hl{超级监管者扩展}\\
 ...
 13 & N & User-level interrupts supported \hl{用户级中断支持}\\
 14 & O & {\em Reserved} \\
 15 & P & {\em Tentatively reserved for Packed-SIMD extension} \\
 16 & Q & Quad-precision floating-point extension \\
 17 & R & {\em Reserved} \\
 18 & S & Supervisor mode implemented \hl{监管者模式实现}\\
 19 & T & {\em Reserved} \\
 20 & U & User mode implemented \hl{用户模式实现}\\
 21 & V & {\em Tentatively reserved for Vector extension} \\
    &   & \hl{暂定向量扩展保留} \\
 22 & W & {\em Reserved} \\
 23 & X & Non-standard extensions present \hl{存在非标准扩展}\\
 24 & Y & {\em Reserved} \\
 25 & Z & {\em Reserved} \\
\hline
\end{tabular}
...
\caption{Encoding of Extensions field in {\tt misa}.  All bits that are
  reserved for future use must return zero when read. ~\\ \hl{{\tt misa}寄存器 
  Extensions字段的编码。所有为了未来的使用所保留的比特在被读取时必须返回0。}}
\label{misaletters}
\end{table*}
...
\hl{
  {\tt misa}寄存器通过机器模式下的编码的方式暴露了一部分基本的CPU特性集。作为 boot 过程的一部分，更多
  详细的信息可以通过在机器模式下检测其他机器寄存器和系统中的只读存储器来获取。
}
...
\hl{
  我们要求通过对低特权等级执行环境的调用而不是直接读取CPU寄存器来确定每一个特权等级可用的CPU特性。
  这使得虚拟层可以改变任一层观察到的ISA，从而支持一个更加丰富的命令接口，而不需要加重硬件设计的负担。
}
\end{commentary}
...
\hl{
  "E"比特是只读的，除非{\tt misa}寄存器被硬布线为0，否则"E"比特总是"I比特的补码。
  同时支持RV32E和RV32I的硬件实现可以通过清除"I"比特来选择RV32E。
}
...
\hl{
  如果指令集的特性{\em x}依赖于特性{\em y}，那么试图使能特性{\em x}并关闭特性{\em y}将导致
  两个特性都被关闭。例如，设置“F=0”且“D=1”将导致“F”和“D”都被清除。
}
...
\hl{
  硬件实现可以对两个或多个{\tt misa}字段的集合施加额外的约束，在这种情况下，它们共同作为一
  个 WARL 字段发挥作用。尝试写入不支持的组合将导致这些位被设置为某些支持的组合。
}
...
\hl{
  写入{\tt misa}，例如关闭"C"扩展，可能会增加指令地址对齐约束。如果一个写入{\tt misa}
  的指令会增加指令地址对齐约束，而且后续的指令地址不满足指令地址对齐约束，那么
  写入{\tt misa}会被抑制，从而保持{\tt misa}不变。
}
...
\hl{
  当软件启用了之前被关闭的某个扩展，此时只与该扩展单独相关的状态是未指定的，
  除非该扩展额外指定了。
}

\subsection{Machine Vendor ID Register {\tt mvendorid} ~\\ 
\hl{机器制造商ID寄存器{\tt mvendorid}}}
...
\hl{
  {\tt mvendorid} CSR是一个32比特的只读寄存器，提供了联合电子设备工程会议
  的处理器核制造商ID。在所有的实现中，这个寄存器都必须是可读的，但是可以返回值
  0来表示该字段没有实现，或者要么这是一个非商业实现。
}
...
\hl{
  {\tt marchid}寄存器是一个MXLEN位的只读寄存器，编码 hart 的基础微架构。这个寄存器在
  任何实现中都必须是可读的，但是可以返回0值来表示该字段没有实现。{\tt mvendorid}和{\tt marchid}
  的组合应该唯一地标识所实现的hart微架构的类型。
}
...
\hl{
  {\tt mimpid}寄存器提供了处理器实现版本的唯一编码。这个寄存器在任何实现中都必须是可读的，
  但是可以返回值0来表示该字段没有实现。实现值应该反映RISC-V处理器本身的设计，而不是任何周围的系统。
}
...
\hl{
  {\tt mhartid} 寄存器是一个 MXLEN 位的只读寄存器，包含正在运行代码的硬件线程的整数ID。
  这个寄存器在任何实现中都必须是可读的。在多处理器系统中，Hart ID不一定是连续编号的，
  但至少有一个Hart ID必须为零。Hart ID在执行环境中必须是唯一的。
}
...
\begin{commentary}
  \hl{
    在某些情况下，我们必须确保存在一个hart来运行一些代码(例如重置)，因此要求一个hart
    有一个已知的hart ID，即为0。为了提高效率，系统实现者应该致力于减少系统中使用的最大hart ID的大小。
  }
\end{commentary}
